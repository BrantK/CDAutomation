...
class BlastTest(unittest.TestCase):
    def test_blasts(self):
        ...
        wd.driver.swipe(h.photo_back_button().location['x']+50, h.photo_back_button().location['y']-50, h.photo_pen().location['x'], h.photo_pen().location['y']+50)
        h.photo_color().click()
        wd.driver.swipe(h.next_button().location['x']+100, h.next_button().location['y']-50, h.location_button().location['x']+50, h.location_button().location['y']+50)
